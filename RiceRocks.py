...
ship_image = simplegui.load_image("https://dl.dropboxusercontent.com/u/172931699/double_ship.png")

# missile image - shot1.png, shot2.png, shot3.png
missile_info = ImageInfo([5,5], [10, 10], 3, 50)
missile_image = simplegui.load_image("https://dl.dropboxusercontent.com/u/172931699/missle(1).png")
...
class Ship:
    ...
    def draw(self,canvas):
        ...
        if self.pos[1] <= 0:
            self.pos[1] = HEIGHT
    def thruster(self,key):
        if key == simplegui.KEY_MAP['up']:
            self.thrust = not my_ship.thrust        
    def turn(self,key,ang):
    ...
    def shoot(self, key):
        ...
        if key == simplegui.KEY_MAP['space']:
            a_missile = Sprite([self.pos[0]+self.forward[0]*self.radius,self.pos[1]+self.forward[1]*self.radius], 
                               [self.vel[0]/100+self.forward[0]*8,self.vel[1]/100+self.forward[1]*8], 
...
class Sprite:
    ...
    def update(self):
        ...
        if a_rock.pos[0] >= WIDTH:
            a_rock.pos[0] = 1
        if a_rock.pos[0] <= 0:
            a_rock.pos[0] = WIDTH
        if a_rock.pos[1] >= HEIGHT:
            a_rock.pos[1] = 1
        if a_rock.pos[1] <= 0:
            a_rock.pos[1] = HEIGHT
...
def rock_spawner():
    ...
    a_rock = Sprite([random.randrange(0,WIDTH), random.randrange(0,HEIGHT)],
                    [random.random()*2*random.choice([-1,1]), random.random()*2*random.choice([-1,1])], 
                    0, random.choice([-random.random()/20,random.random()/20]), 
                    asteroid_image, asteroid_info)
    
#experimental key handler
def keydown(key):
    my_ship.thruster(key)
    my_ship.turn(key,.15)
    my_ship.shoot(key)
def keyup(key):
    my_ship.thruster(key)
...
a_rock = Sprite([WIDTH / 3, HEIGHT / 3], [random.random()*3, random.random()*3], 0, 0, asteroid_image, asteroid_info)
